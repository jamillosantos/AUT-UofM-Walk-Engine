...
void vDCM_Update_Task( void *pvParameters ){ 
  ...
  for(int i=0;i<=3;i++){ 
      ...
      Dxl.writeByte(Id_Right_Hip_Roll,P_P_Gain,30);
      Dxl.writeByte(Id_Left_Hip_Roll,P_P_Gain,30);
      
      Dxl.writeByte(Id_Right_Foot_Roll,P_P_Gain,30);
      Dxl.writeByte(Id_Left_Foot_Roll,P_P_Gain,30);
  ...
  for( ;; ){
    ...
    if(SerialUSB.available()==9){
      ...
      if(buffer[0]==254){
                //walk data update
                double vx=((double) ((((byte)buffer[1])-100) / 100.0));
                double vy=((double) ((((byte)buffer[2])-100) / 100.0));
                double vt=((double) ((((byte)buffer[3])-100) / 100.0));
                ...
                if ((Internal_Motion_Request==No_Motion ) && (Motion_Ins==No_Motion)){
                  //head data update
                  unsigned long T=0;
