...
void vDCM_Update_Task(void *pvParameters)
{
	...
	vTaskSuspendAll();
	Init_Dxls_First_Time();  //initialize the array of the actuators

	//initialize GY80 power pins
	pinMode(MPU_GY80_VCC_Pin, OUTPUT);
	digitalWrite(MPU_GY80_VCC_Pin, HIGH);  //gnd of mpu-GY80
	pinMode(MPU_GY80_GND_Pin, OUTPUT);
	digitalWrite(MPU_GY80_GND_Pin, LOW);   //vcc of mpu-GY80
	vTaskDelay(10);
	...
	//for the first time init the value of the dynamixle
	vTaskDelay(10);
	DXL_Write_PS();
	vTaskDelay(10);

	//init the MPU value (Roll, Pitch, Yaw)
	MPU_X = 0.0;
	MPU_Y = 0.0;
	MPU_Z = 0.0;
	...
	for (int i = 0; i <= 3; i++)
	{
		...
		Dxl.writeByte(BROADCAST_ID, P_Status_Return_Level, 1);

		xTaskResumeAll();
		vTaskDelay(5);
	}

	//calculate the Euler angles for the first time that need for more stability af first time
...
//update head actuators
void DXL_Write_Head()
{
	vTaskSuspendAll();
	Dxl.setPosition(Id_Head_Pan, 2048 + (((int) ((Head_Pan_Angle * RAD2DEG) * DEG2DXL)) * 1), (int) Head_Pan_Speed);
	Dxl.setPosition(Id_Head_Tilt, 2048 + (((int) (((Head_Tilt_Angle - MPU_X) * RAD2DEG) * DEG2DXL)) * 1),
					(int) Head_Tilt_Speed);
	xTaskResumeAll();
}
...
void DXL_Write_PS()
{
	...
	for (byte i = 0; i <= NUM_OF_DXL - 1; i++)
	{
		...
		D_GOAL_POSITION[i] = 2048 + (((int) ((Angle[i] * RAD2DEG) * DEG2DXL)) *
									 (int) Joints_Direction[i]);  //init goal position for first time
...
void Calculate_Euler_Angles()
{
	...
	MPU_Angle_X = (kalmanX.getAngle(
		(atan(MPU.get_Az() / sqrt((MPU.get_Ax() * MPU.get_Ax()) + (MPU.get_Ay() * MPU.get_Ay()))) * RAD_TO_DEG),
		-MPU.get_Gx(), dt)) * DEG2RAD; // Calculate the angle using a Kalman filter
